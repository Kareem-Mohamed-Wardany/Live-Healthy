...
class ImageProcessing:
    ...
    def GenerateDataSet(self):
        dsM = Dataset()
        data = None
        if (os.path.exists('Data/DatasetGray.npy')): # If you have already created the dataset:
            data = dsM.loadDataset()
        else: # If dataset is not created:
            folders = [os.path.join(self.root_folder, x) for x in ('COVID/train/', 'Fibrosis/train/', 'Normal/train/', 'Tuberculosis/train/', 'PNEUMONIA/train/')] # create a list contains all sub folders in our root
            data = [img for folder in folders for img in self.load_images_from_folder(folder)] # get images in each subfolder
            shuffle(data) # shuffle arrangement of dataset
            dsM.saveDataset("Data/DatasetGray.npy", data) # Save dataset to be easily used later
        images = np.array([i[0] for i in data]) # holds the images from dataset into this variable
        labels = [i[1] for i in data] # holds the labels for each image into this variable
        labels = keras.utils.to_categorical(labels, 6) # Converts a class vector (integers) to binary class matrix for example COVID = 1 => [0. 1. 0. 0. 0. 0.]
